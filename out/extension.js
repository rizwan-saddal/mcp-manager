...
async function registerMcpTools(context) {
  try {
    const configPath = getMcpConfigPath();
    ...
    for (const [serverId, serverConfig] of Object.entries(servers)) {
      if (serverId === "brave") {
        registerBraveTools(context, serverConfig);
      } else if (serverId === "fetch") {
        registerFetchTools(context, serverConfig);
      }
    }
  } catch (e) {
...
function registerBraveTools(context, config) {
  const tools = [
    {
      name: "brave_search",
...
function registerFetchTools(context, config) {
  const tools = [
    {
      name: "fetch",
...
async function runMcpToolCall(config, toolName, args) {
  return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (match) {
          ...
          if (response.result && response.result.content) {
            const content = response.result.content.map((c) => c.text || JSON.stringify(c)).join("\n");
            resolve(content);
